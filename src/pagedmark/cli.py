...
from pagedmark import __version__, image_io, watermark_registry
...
from pagedmark.video import VIDEO_VISIBLE_MARKS
from pagedmark.video_synthid import (
...
if TYPE_CHECKING:
    ...
    from pagedmark.api import InvisibleOptions
...
def _banner() -> None:
    console.print(
        Panel(
            f"pagedMark v{__version__}\nAI watermark removal with a Metal backend",
            border_style="cyan",
            padding=(0, 2),
        )
    )
...
def _invisible_install_hint() -> str:
    """The extra that makes a removal run work on THIS machine.

    Naming ``qwen-zimage`` on a Mac is an install hint that does not install: the extra
    pulls DiffSynth for a stack Metal cannot load. Same rule as the shell quoting --
    a printed command has to be the one that works when pasted.
    """
    from pagedmark._internal.watermark_profiles import install_extra_for_device
    from pagedmark._internal.watermark_remover import get_device

    return install_extra_for_device(get_device())


def _require_invisible_dependencies() -> None:
    """Exit with the device-appropriate install hint when the runtime is missing."""
    from pagedmark.invisible_engine import is_available as invisible_available

    if not invisible_available():
        console.print(
            "Error: the invisible-removal dependencies are not installed.\n"
            f"  Install them with: pip install {_invisible_install_hint()}"
        )
        raise SystemExit(1)
...
# Each profile pins a fixed model stack, a distilled per-stage schedule and CFG 1.0;
# every one of those knobs existed only so the library could reject it several layers
# down. A flag whose sole outcome is an error is worse than no flag at all -- it
# advertises a capability that does not exist. --device stays out for the same reason
# it always did: auto-detection already finds the one device that can run, and now
# that MPS is supported, the profile follows from it rather than the other way round.
...
_PIPELINE_HELP = (
    "Pipeline profile. qwen-zimage = Qwen-Image-2512 + Lightning + Canny, followed by "
    "SAM-masked Z-Image face repair (CUDA-ONLY: its stack streams float8 weights, which "
    "Metal does not implement); sdxl-zimage = the same recipe and the same face stage on "
    "an SDXL global pass, which needs more denoise, and the only profile that also runs "
    "on Apple Silicon. Default: per device -- qwen-zimage on CUDA, sdxl-zimage on MPS, "
    "where the face stage is skipped and the global stage runs alone."
)
...
_pipeline_option = click.option(
    ...
    # Unset rather than a literal default: "the user did not choose" and "the user
    # typed qwen-zimage" deserve different answers on a Mac -- auto-select the profile
    # that runs, versus an error explaining why that one cannot.
    default=None,
    help=_PIPELINE_HELP,
)
...
def _visible_provenance(path: Path | None) -> frozenset[str]:
    ...
    if path is None:
        return frozenset()
    from pagedmark.api import visible_provenance
...
def _no_visible_mark_exit(source: Path) -> NoReturn:
    ...
    if report.is_ai_generated and report.watermarks:
        ...
        console.print(
            ...
            f"    pagedmark erase {source.name} --region x,y,w,h"
        )
    console.print(
        ...
        f"    pagedmark visible {source.name} --mark doubao --no-detect"
    )
...
def _no_invisible_signal_exit(source: Path) -> NoReturn:
    ...
    console.print(
        ...
        f"    pagedmark invisible {source.name} --force"
    )
...
def _should_skip_invisible_scrub(force: bool, image_path: Path) -> bool:
    ...
    if force:
        return False
    from pagedmark.identify import has_invisible_target
...
@click.version_option(__version__, prog_name="pagedmark")
...
def cmd_invisible(
    ...
    Regenerates the pixels with the diffusion profile the device can run. NVIDIA:
    pip install 'pagedmark[qwen-zimage]'. Apple Silicon:
    pip install 'pagedmark[diffusion]'
    """
    _require_invisible_dependencies()

    from pagedmark.invisible_engine import InvisibleEngine
    ...
    # A device/profile mismatch is a user-facing condition (no GPU, or qwen-zimage
    # asked for on a Mac), and the message already says what to do about it. Printing
    # it beats a traceback from three frames down.
    try:
    ...
    # The engine's resolved profile, not the request: with --pipeline unset the device
...
def cmd_metadata(
    ...
    """
    from pagedmark.metadata import get_ai_metadata, has_ai_metadata, strip_and_verify
...
def cmd_video_identify(source: Path, no_visible: bool, as_json: bool) -> None:
    ...
    from pagedmark.video import identify_video
...
def cmd_video_metadata(
    ...
    from pagedmark.video import inspect_video_metadata, remove_video_metadata
...
def cmd_video_invisible(
    ...
    from pagedmark.video import remove_video_invisible
...
def cmd_video_visible(
    ...
    from pagedmark.video import remove_video_visible
...
def cmd_video_all(
    ...
    from pagedmark.video import remove_video_all
...
def cmd_video_batch(
    ...
    from pagedmark.video import remove_video_batch
...
def cmd_all(
    ...
    from pagedmark.api import InvisibleOptions, MetadataStripIncomplete, remove_all
    ...
    if outcome.invisible == "unavailable":
        console.print(
            ...
            f"    pip install {_invisible_install_hint()}\n"
            "  ====================================================================="
        )
        raise SystemExit(1)
...
def _batch_engine(mode: str, options: InvisibleOptions) -> object | None:
    ...
    if mode not in ("all", "invisible"):
        return None
    from pagedmark.invisible_engine import InvisibleEngine, is_available
...
def cmd_batch(
    ...
    from pagedmark.api import InvisibleOptions
    from pagedmark.api import remove_batch as api_remove_batch
    ...
    if synthid_skipped_count:
        ...
        console.print(
            ...
            f"  Install the extra and rerun: pip install {_invisible_install_hint()}"
        )
