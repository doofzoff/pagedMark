...
def _prepare_sam_inputs(inputs: Any, device: str, dtype: Any) -> Any:
    ...
    # The processor emits its box/point prompts as float64, and Metal has no float64
    # type at all -- moving the batch as-is raises rather than degrading. Cast those
    # down FIRST; on CUDA this is a no-op in effect, since the prompts are geometry.
...
def _clip_sam_masks_to_boxes(
    ...
    from pagedmark.region_eraser import boxes_to_mask
...
class QwenZImagePipeline:
    ...
    face_stage: bool = True

    # Every device this profile's stack can load on. Overridden by the SDXL profile,
    # whose global stage is plain Diffusers. Sourced from the same table the remover's
    # precondition reads, so a pipeline constructed directly cannot be more permissive
    # than the gate in front of it.
    supported_devices: ClassVar[tuple[str, ...]] = PROFILE_DEVICES[QWEN_ZIMAGE_PROFILE]
    ...
    def _require_cuda(self) -> None:
        """Guard the DiffSynth stacks, which stream float8 weights Metal cannot hold."""
        if self.device != CUDA_DEVICE:
            raise RuntimeError(
                f"This model stack is CUDA-only; '{self.device}' cannot load it. Its "
                "float8 storage dtype has no Metal implementation. WatermarkRemover "
                "already refuses an unsupported device, so reaching this guard means "
                "the pipeline was constructed directly."
            )

    def _require_supported_device(self) -> None:
        """Guard the run itself against a device this profile's global stage cannot use."""
        if self.device not in self.supported_devices:
            raise RuntimeError(
                f"{type(self).__name__} cannot run on '{self.device}'. Supported: {', '.join(self.supported_devices)}."
            )

    def _device_memory_gib(self) -> float:
        """Working-set budget for this device, 0.0 when it cannot be read."""
        from pagedmark._internal.watermark_remover import device_memory_gib

        return device_memory_gib(self.device)

    def _should_tile(self, image: Image.Image, tile: bool, tile_size: int) -> bool:
        """Whether to regenerate in overlapping tiles instead of one forward pass.

        The base answer is the caller's request. Overridden by profiles that can also
        reach the decision from a device budget.
        """
        return tile and max(image.size) > tile_size
    ...
    def _load_qwen(self) -> Any:
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install: pip install 'pagedmark[qwen-zimage]'"
            ) from exc
    ...
    def _sam_masks(
        ...
        try:
            ...
            clipped = _clip_sam_masks_to_boxes(binary_masks, boxes, image.size)
            self._release_sam_if_transient()
            return clipped
    ...
    def _release_sam_if_transient(self) -> None:
        """Drop SAM once its masks are numpy, where holding it costs more than reloading.

        Default: keep it, which is what a card with headroom wants across a batch.
        """

    def _face_crop_cap(self) -> int | None:
        """Largest side a face crop is processed at, or None for the profile's own.

        A cap is a device concern rather than a quality one, so it lives next to the
        stage that pays for it instead of inside ``_detail_size``.
        """
        return None
    ...
    def _regenerate_face_crop(
        ...
        *,
        ...
        """Regenerate one face crop. Overridden where Z-Image cannot be loaded."""
        pipe = self._load_zimage()
        self._progress(
            f"Regenerating face {index}/{total} with Z-Image: strength={strength:.4f}, steps={FACE_STEPS}..."
        )
        return pipe(**build_face_kwargs(crop, strength=strength, seed=seed))
    ...
    def _run_faces(
        ...
        *,
        ...
        for index, (box, mask) in enumerate(zip(boxes, masks, strict=True), start=1):
            ...
            cap = self._face_crop_cap()
            if cap is not None and max(process_size) > cap:
                scale = cap / max(process_size)
                process_size = (
                    max(16, round(process_size[0] * scale / 16) * 16),
                    max(16, round(process_size[1] * scale / 16) * 16),
                )
    ...
    def run(
        ...
        *,
        ...
        """Execute global regeneration and, where the device allows it, masked face repair."""
        ...
        if self._should_tile(image, tile, tile_size):
            from pagedmark._internal.tiling import run_tiled
        ...
        if not self.face_stage:
            # Detection is skipped too, not just the repair: running YuNet only to
            # report faces this device will not touch costs a model download and
            # reads as a stage that ran.
            self._progress("Face repair is unavailable on this device; keeping the global result.")
            boxes = []
        else:
