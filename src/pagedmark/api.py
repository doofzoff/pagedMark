...
Imports stay lazy (inside the functions), so ``import pagedmark`` is cheap.
"""
...
if TYPE_CHECKING:
    ...
    from pagedmark.watermark_registry import Backend, Sensitivity
...
def visible_provenance(source: str | Path) -> frozenset[str]:
    ...
    with contextlib.suppress(Exception):
        from pagedmark import identify
...
def _tc260_vendors(path: Path) -> frozenset[str]:
    ...
    from pagedmark._internal.constants import TC260_FALLBACK_VENDORS

    with contextlib.suppress(Exception):
        from pagedmark.metadata import aigc_label, uscc_of
        from pagedmark.watermark_registry import tc260_producer_vendors
...
def _write_visible_result(
    ...
    *,
    ...
    if strip_metadata:
        from pagedmark import metadata
...
def remove_visible(
    ...
    *,
    ...
    """
    from pagedmark import watermark_registry
...
class InvisibleOptions:
    ...
    # None, mirroring the engine: the profile follows the resolved device (qwen-zimage
    # on CUDA, sdxl-zimage on MPS, which is the only one Metal can load). A literal
    # here would make every library caller pin the CUDA profile by accident.
...
class _SourceEvidence:
    ...
    def _extract(self) -> Any | None:
        if not self._extracted:
            ...
            with suppress(Exception):
                from pagedmark.identify import extract_provenance_evidence
    ...
    def visible_provenance(self) -> frozenset[str]:
        ...
        with suppress(Exception):
            from pagedmark.identify import identify_from_evidence
    ...
    def has_invisible_target(self) -> bool:
        ...
        if evidence is None:
            return True
        try:
            from pagedmark._internal.c2pa import c2pa_info_has_removal_hint
            from pagedmark.identify import identify_from_evidence
...
def _provenance_from_report(report: Any, path: Path) -> frozenset[str]:
    ...
    """
    from pagedmark.watermark_registry import known_marks
...
def remove_all(
    ...
    *,
    ...
    from pagedmark.metadata import strip_and_verify
...
def _run_invisible(
    ...
    from pagedmark._internal.watermark_profiles import resolve_strength, vendor_for_strength
    ...
    # resolves it, so the reported value cannot drift from the executed one. Built
    # AFTER the engine because the strength policy is per-profile and the profile is
    # only known once the device has chosen it: with ``pipeline=None`` on a Mac, this
    # read from the request would report the CUDA profile's resolution curve for a run
    # that executes the SDXL profile's vendor policy.
    with suppress(Exception):
        from PIL import Image

        with Image.open(vendor_source) as probe:
            say("invisible", f"strength={resolve_strength(opts.strength, vendor, engine.profile, size=probe.size)}")
