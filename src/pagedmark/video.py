...
if TYPE_CHECKING:
    from pagedmark.video_invisible import RegenerationMetrics, VideoVaeRuntime
    from pagedmark.video_visible import VideoScan
...
def _require_video_runtime() -> None:
    ...
    if not module_available("cv2", "numpy", "av"):
        raise RuntimeError("Video pixel processing requires pagedmark[video]")
...
def _visible_removal_plan(
    ...
    """
    from pagedmark.video_visible import (
...
def _platform_from_video_metadata(markers: dict[str, str]) -> str | None:
    ...
    from pagedmark._internal.constants import C2PA_AI_VENDORS
...
def identify_video(
    ...
    *,
    ...
    if check_visible:
        ...
        from pagedmark.video_visible import scan_video_marks
...
def remove_video_metadata(
    ...
    *,
    ...
    """
    from pagedmark.metadata import get_ai_metadata, strip_and_verify
...
def remove_video_visible(
    ...
    *,
    ...
    from pagedmark.metadata import get_ai_metadata
    from pagedmark.video_visible import encode_clean_video, scan_video_marks
    from pagedmark.watermark_registry import resolve_backend
...
def remove_video_batch(
    ...
    *,
    ...
    from pagedmark.video_encoding import atomic_video_output
        ...
        try:
            if mode == "all":
                if (
                    ...
                    from pagedmark.video_invisible import load_video_vae_runtime
...
def remove_video_invisible(
    ...
    *,
    ...
    if source_path.suffix.lower() not in _REGENERATED_VIDEO_EXTENSIONS:
        ...
        raise ValueError(f"Video SynthID regeneration requires one of: {supported}")
    _require_video_runtime()
    from pagedmark.video_invisible import regenerate_video_candidate
