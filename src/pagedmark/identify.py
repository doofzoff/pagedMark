...
from pagedmark._internal.schema import require_schema_version
...
from pagedmark.watermark_registry import known_marks as _known_marks
...
def evidence_from_metadata_record(
    ...
    """
    from pagedmark.metadata_record import METADATA_RECORD_SCHEMA_VERSION, METADATA_RECORD_TYPE
...
def _visible_text_marks(image_path: Path, *, image: NDArray[Any] | None = None) -> list[MarkDetection]:
    ...
    """
    try:
        from pagedmark.image_io import imread
        from pagedmark.watermark_registry import get_mark
    except Exception as exc:  # cv2/engine assets missing
...
def _invisible_watermark(image_path: Path, decode: _SharedDecode) -> str | None:
    ...
    """
    from pagedmark.invisible_watermark import detect_invisible_watermark, is_available
...
def _trustmark(image_path: Path) -> str | None:
    ...
    """
    from pagedmark.trustmark_detector import detect_trustmark
