...
def engine_for(mark_key: str) -> Any:
    ...
    mod = importlib.import_module(f"pagedmark.{mark_key}_engine")
    cls = next(
        obj
...
def stamp(
    ...
    """
    from pagedmark._text_mark_engine import load_alpha_template
...
def _slot_alpha(mark_key: str) -> np.ndarray | None:
    ...
    if mark_key == "gemini":
        from pagedmark.gemini_engine import _shared_engine
...
def clean_sources(n: int, seed: int = 11) -> list[Path]:
    ...
    from pagedmark.watermark_registry import detect_marks
...
def main() -> None:
    ...
    from pagedmark.image_io import imread
    from pagedmark.region_eraser import lama_available, migan_available
    from pagedmark.watermark_registry import fill, get_mark
