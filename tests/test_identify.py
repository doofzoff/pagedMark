...
from pagedmark.identify import (
    ProvenanceEvidence,
    ProvenanceReport,
    _ai_tools_in,
    _attribute_platform,
...
from pagedmark.watermark_registry import GEMINI_SPARKLE_TRUST_CONF

# Where the lazy import inside identify._visible_sparkle resolves the detector.
_SPARKLE_TARGET = "pagedmark.gemini_engine.detect_sparkle_confidence"
...
class TestProvenanceEvidence:
    ...
    def test_identify_from_evidence_does_not_read_the_source(self, monkeypatch, tmp_path: Path):
        ...
        monkeypatch.setattr("pagedmark.identify.extract_c2pa_info", fail_if_called)
        monkeypatch.setattr("pagedmark.identify.get_ai_metadata", fail_if_called)
        monkeypatch.setattr("pagedmark.identify.scan_head", fail_if_called)
        monkeypatch.setattr("pagedmark.identify.iptc_ai_system", fail_if_called)
        monkeypatch.setattr("pagedmark.identify.aigc_label", fail_if_called)
        monkeypatch.setattr("pagedmark.identify.exif_generator", fail_if_called)
        monkeypatch.setattr("pagedmark.identify.xai_signature", fail_if_called)
        monkeypatch.setattr("pagedmark.identify.huggingface_job", fail_if_called)
        monkeypatch.setattr("pagedmark.identify.samsung_genai", fail_if_called)
...
class TestIdentifyNonPng:
    ...
    def test_claim_generator_wins_over_upstream_issuer(
        ...
        monkeypatch.setattr(
            "pagedmark.identify.extract_c2pa_info",
            lambda _path: {
                "has_c2pa": True,
                "issuer": "OpenAI",
...
class TestHasInvisibleTargetFailSafe:
    ...
    def test_detector_error_defaults_to_run(self, tmp_path: Path):
        ...
        with patch("pagedmark.identify._identify_from_evidence", side_effect=RuntimeError("boom")):
...
class TestSparkleDetectRemoveAlignment:
    ...
    def _detect_remove(self, path: Path) -> tuple[bool, bool, float]:
        from pagedmark import image_io, watermark_registry
        from pagedmark.gemini_engine import detect_sparkle_confidence
...
class TestIdentifyImportIsLight:
    ...
    def test_import_identify_does_not_pull_torch(self):
        ...
        code = "import sys, pagedmark.identify; sys.exit(1 if 'torch' in sys.modules else 0)"
...
_TEXT_MARKS_TARGET = "pagedmark.identify._visible_text_marks"
...
class TestIdentifyVisibleTextMarks:
    ...
    def _detection(key: str, label: str, conf: float):
        from pagedmark.watermark_registry import MarkDetection
    ...
    def test_visible_path_decodes_file_once(self, tmp_clean_png: Path):
        ...
        import pagedmark.image_io as image_io
...
from pagedmark.invisible_watermark import is_available as _wm_available  # noqa: E402
...
class TestIdentifyInvisibleWatermark:
    def _sdxl_watermarked(self, tmp_path: Path) -> Path:
        ...
        from pagedmark.invisible_watermark import _BITS_48
...
class TestSharedPixelDecode:
    ...
    def test_a_decode_failure_still_propagates_on_the_invisible_arm(self):
        ...
        from pagedmark import identify as identify_mod
        from pagedmark import image_io
...
class TestSynthIdProxyIsDecidedInTheVerdict:
    ...
    def test_both_paths_infer_it_from_the_same_bytes(self, tmp_path: Path):
        from pagedmark.identify import identify_metadata_record
        from pagedmark.metadata_record import collect_metadata_record
...
class TestRegistryScansSkipTheCodedPixels:
    ...
    def test_a_token_in_a_marker_segment_is_attributed(self, tmp_path: Path):
        from pagedmark.identify import _issuers_in, _metadata_region
        from pagedmark.metadata import scan_head
    ...
    def test_a_container_that_does_not_parse_is_left_whole(self, tmp_path: Path):
        ...
        from pagedmark.identify import _metadata_region
