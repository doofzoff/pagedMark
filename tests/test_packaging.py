...
# The published distribution name. A literal, and deliberately so: these tests assert
# what the built metadata says, and deriving the name from that same metadata would
# make them agree with themselves. A rename has to break this line and be re-read.
DISTRIBUTION = "pagedmark"
...
def _requirement_names(extra: str | None = None, *, python_version: str | None = None) -> set[str]:
    ...
    parsed = (Requirement(value) for value in requires(DISTRIBUTION) or [])
    return {
...
def test_supported_python_floor_is_published():
    assert SpecifierSet(metadata(DISTRIBUTION)["Requires-Python"]) == SpecifierSet(">=3.11,<3.15")
...
def test_extras_use_capability_names_without_legacy_aliases():
    extras = set(metadata(DISTRIBUTION).get_all("Provides-Extra") or [])
