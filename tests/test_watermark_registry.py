...
from pagedmark import watermark_registry as reg
...
class TestEngineImportsStayLazy:
    ...
    def test_importing_the_registry_pulls_no_engine_and_no_cv2(self):
        loaded = self._modules_after("import pagedmark.watermark_registry")
        assert "pagedmark.gemini_engine" not in loaded
        assert "pagedmark.doubao_engine" not in loaded
    ...
    def test_importing_identify_pulls_no_engine_and_no_cv2(self):
        ...
        loaded = self._modules_after("import pagedmark.identify")
        assert "pagedmark.gemini_engine" not in loaded
        assert "cv2" not in loaded

    def test_resolving_an_engine_imports_it(self):
        loaded = self._modules_after("from pagedmark import watermark_registry as r\nr._engine('gemini')")
        assert "pagedmark.gemini_engine" in loaded
...
class TestProvenanceGate:
    ...
    def _stub(self, monkeypatch: pytest.MonkeyPatch, conf: float) -> None:
        from pagedmark.gemini_engine import DetectionResult
...
class TestMarkKnowledgeIsOnTheRow:
    ...
    def test_identify_platform_table_is_derived_from_the_rows(self):
        from pagedmark.identify import _VISIBLE_MARK_PLATFORM
